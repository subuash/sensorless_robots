...
class MonteCarlo():
    ...
    def resample(self, sampleSize, prev_diff, recursion_limit):
        ...
        for _ in range(sampleSize):
            ...
            while True:
                x = int(point.path[-1][0] + np.random.uniform(-spawn_bounds, spawn_bounds))
                y = int(point.path[-1][1] + np.random.uniform(-spawn_bounds, spawn_bounds))

                if 0 <= x < self.width and 0 <= y < self.height and self.obstacles[y, x] == 0:
                    break
            var = 0
            if self.var_random_angle != 0:
